...
### AGENT-CHROME-019：跨年帖子时间显示年份

范围：
时间格式回归，2026-03-30 新增

状态：
当前不在默认批量脚本中；需要分别准备一个今年帖子和一个非今年帖子
...
1. 今年的时间文本只显示月、日、时、分，不带年份
2. 非今年的时间文本必须显示四位年份
3. 两类时间文本都不允许因为年份缺失而无法区分是否跨年

### AGENT-CHROME-020：回复面板 emoji shortcode 搜索插入

范围：
抽屉快速回复面板 emoji 面板与 shortcode 插入

状态：
当前不在默认批量脚本中；用例内使用扩展自带回复面板
...
3. 点击工具栏中的 `表情`
4. 在搜索框输入 `grinning`
5. 点击 `:grinning_face:` 对应表情

断言：

1. emoji 面板成功展开，并显示搜索框、分类栏和表情网格
2. 搜索 `grinning` 后，结果里应包含 `:grinning_face:`
3. 点击表情后，回复输入框会在当前光标位置插入 `:grinning_face:`
4. 插入完成后 emoji 面板会自动关闭，不影响后续继续输入或发送

### AGENT-CHROME-021：emoji 分类点击与滚动联动

范围：
回复面板 emoji 面板分类布局与滚动高亮

状态：
当前不在默认批量脚本中；需要打开扩展自带回复面板
...
1. 默认视图下，右侧表情按分类分段显示，而不是单一分类网格
2. 点击左侧分类后，右侧内容会滚动到对应分类分组
3. 右侧内容滚动到新分组后，左侧高亮分类会自动切换
4. 搜索模式下不应再强制跟随分类滚动联动，避免搜索结果视图乱跳

### AGENT-CHROME-022：emoji 远端数据优先与本地回退

范围：
回复面板 emoji 数据源策略

状态：
当前不在默认批量脚本中；需要打开扩展自带回复面板，并能观察网络请求
...
2. 点击头部 `回复主题` 打开回复面板
3. 点击工具栏中的 `表情`
4. 观察是否发起 `/emojis.json` 与 `/emojis/search-aliases.json`
5. 在网络失败或被拦截时，重新打开表情面板

断言：

1. 首次打开 emoji 面板时，应优先尝试加载论坛当前 `/emojis.json`
2. 接口成功时，面板应能展示论坛实际表情集，而不只是一小撮硬编码表情
3. 接口失败时，面板仍应显示本地兜底表情，而不是空白或报错卡死
4. 无论远端还是本地数据源，点击表情后都应插入对应 shortcode
